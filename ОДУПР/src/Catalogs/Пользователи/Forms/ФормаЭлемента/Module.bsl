...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//КТ-2000 Ежов Д.С. 10.07.2011
	алкЗаполнитьДеревоНастроек();
	алкЗаполнитьДеревоДопПрав();
	//КТ-2000 Ежов Д.С. 10.07.2011
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//КТ-2000 Ежов Д.С. 10.07.2011
	алкЗаписатьДеревоНастроек();
	алкЗаписатьДеревоДопПрав();
	Если ТекущийОбъект.Ссылка = ПараметрыСеанса.ТекущийПользователь Тогда
...
Процедура ФизическоеЛицоПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ФизическоеЛицо) Тогда
		...
		ИначеЕсли Объект.Наименование <> Строка(Объект.ФизическоеЛицо) Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ФизическоеЛицоПриИзмененииЗавершение", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
#КонецОбласти
//КТ-2000 Ежов Д.С. 10.07.2011 Работа с настройками пользователя

&НаКлиенте
Процедура алкОбновитьНастройкиПользователя(Команда)
...
&НаСервере
Процедура алкЗаполнитьДеревоНастроек()
	
	ДеревоНастроек = РеквизитФормыВЗначение("алкДеревоНастроек");
	...
	|ИЗ
	|	ПланВидовХарактеристик.алкНастройкиПользователей КАК Настройки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.алкНастройкиПользователей КАК ЗначениеНастроек
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Значение = NULL Тогда
			СтрокаНастройки.Значение = Выборка.Ссылка.ТипЗначения.ПривестиЗначение(Неопределено);
		Иначе
	...
	КонецЦикла;
	ЗначениеВРеквизитФормы(ДеревоНастроек, "алкДеревоНастроек");
	
КонецПроцедуры

&НаСервере
Процедура алкЗаполнитьДеревоДопПрав()

	ДеревоДопПрав = РеквизитФормыВЗначение("алкДеревоДопПрав");
	ДеревоДопПрав.Строки.Очистить();
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Права.Родитель,
	...
	|ИЗ
	|	ПланВидовХарактеристик.алкПраваПользователей КАК Права
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.алкЗначенияДополнительныхПравПользователя КАК ЗначениеПрав
	|		ПО (ЗначениеПрав.Право = Права.Ссылка)
	|			И (ЗначениеПрав.Пользователь = &Пользователь)
	|ГДЕ
	|	(НЕ Права.ПометкаУдаления)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЭтоГруппа ИЕРАРХИЯ,
	|	Права.Наименование";
	...
	Пока Выборка.Следующий() Цикл
		Если Не Выборка.Родитель.Пустая() Тогда
			СтрокаГруппы = ДеревоДопПрав.Строки.Найти(Выборка.Родитель, "Право", Истина);
			Если СтрокаГруппы = Неопределено Тогда
				СтрокаГруппы = ДеревоДопПрав.Строки.Добавить();
				СтрокаГруппы.Право = Выборка.Родитель;
				СтрокаГруппы.ЭтоГруппа = Выборка.ЭтоГруппа;
			КонецЕсли;
		Иначе
			СтрокаГруппы = ДеревоДопПрав;
		КонецЕсли;
		СтрокаНастройки = СтрокаГруппы.Строки.Добавить();
		СтрокаНастройки.Право = Выборка.Ссылка;
		Если Выборка.Значение = NULL Тогда
			СтрокаНастройки.Значение = Выборка.Ссылка.ТипЗначения.ПривестиЗначение(Неопределено);
		Иначе
			СтрокаНастройки.Значение = Выборка.Ссылка.ТипЗначения.ПривестиЗначение(Выборка.Значение);
		...
		СтрокаНастройки.ЭтоГруппа = Выборка.ЭтоГруппа;
	КонецЦикла;
	ЗначениеВРеквизитФормы(ДеревоДопПрав, "алкДеревоДопПрав");
	
КонецПроцедуры

&НаСервере
Функция алкЗаполнитьНаборЗаписей(СтрокиДерева, НаборЗаписей, ИмяПоля)
	
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		Если Не СтрокаДерева.ЭтоГруппа Тогда
			Если СтрокаДерева[ИмяПоля].ПометкаУдаления Тогда
				Продолжить;
			КонецЕсли;
			Запись = НаборЗаписей.Добавить();
			Запись.Пользователь = Объект.Ссылка;
			Запись[ИмяПоля] = СтрокаДерева[ИмяПоля];           
			Запись.Значение = СтрокаДерева[ИмяПоля].ТипЗначения.ПривестиЗначение(СтрокаДерева.Значение);
			Если Не Объект.Ссылка.Пустая()
				И Запись[ИмяПоля] = ПланыВидовХарактеристик.алкНастройкиПользователей.ОсновнойМенеджер 
				И Запись.Значение.Пустая() Тогда
				Запись.Значение = Объект.Ссылка;
				СтрокаДерева.Значение = Объект.Ссылка;
			КонецЕсли;
		Иначе
			алкЗаполнитьНаборЗаписей(СтрокаДерева.Строки, НаборЗаписей, ИмяПоля);
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура алкЗаписатьДеревоНастроек()
	
	Набор = РегистрыСведений.алкНастройкиПользователей.СоздатьНаборЗаписей();
	Набор.Отбор.Пользователь.Использование = Истина;
	Набор.Отбор.Пользователь.Значение = Объект.Ссылка;
	ДеревоНастроек = РеквизитФормыВЗначение("алкДеревоНастроек");
	Если алкЗаполнитьНаборЗаписей(ДеревоНастроек.Строки, Набор, "Настройка") Тогда
		Набор.Записать();
	КонецЕсли;
	ЗначениеВРеквизитФормы(ДеревоНастроек, "алкДеревоНастроек");
...
&НаСервере
Процедура алкЗаписатьДеревоДопПрав()
	
	Набор = РегистрыСведений.алкЗначенияДополнительныхПравПользователя.СоздатьНаборЗаписей();
	Набор.Отбор.Пользователь.Использование = Истина;
	Набор.Отбор.Пользователь.Значение = Объект.Ссылка;
	ДеревоДопПрав = РеквизитФормыВЗначение("алкДеревоДопПрав");
	Если алкЗаполнитьНаборЗаписей(ДеревоДопПрав.Строки, Набор, "Право") Тогда
		Набор.Записать();
	КонецЕсли;
	ЗначениеВРеквизитФормы(ДеревоДопПрав, "алкДеревоДопПрав");
...
&НаКлиенте
Процедура алкДеревоНастроекЗначениеОткрытие(Элемент, СтандартнаяОбработка)
	
	Если Элементы.алкДеревоНастроек.ТекущиеДанные <> Неопределено Тогда
		Если Элементы.алкДеревоНастроек.ТекущиеДанные.Настройка = ПредопределенноеЗначение("ПланВидовХарактеристик.алкНастройкиПользователей.алкПерсональнаяНастройкаСвязиСЛК") Тогда
			СтандартнаяОбработка = Ложь;
			СтруктураПараметров = Новый Структура;
			СтруктураПараметров.Вставить("ТекущийПользователь", Объект.Ссылка);
			ОписаниеОповещения = Новый ОписаниеОповещения("алкДеревоНастроекЗначениеОткрытиеЗавершение", ЭтотОбъект); 
			ОткрытьФорму("Обработка.алкЗащитаИЛицензированиеАлкогольнойПодсистемы.Форма.алкОсобыеНастройкиДоступаКСерверуСЛК", СтруктураПараметров, ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура алкДеревоНастроекЗначениеОткрытиеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено Тогда
		Элементы.алкДеревоНастроек.ТекущиеДанные.Значение = Результат;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура алкОбновитьДопПраваПользователя(Команда)
...
&НаКлиенте
Процедура алкРазвернутьДерево()
	
	Для Каждого ЭлементСтроки Из алкДеревоНастроек.ПолучитьЭлементы() Цикл
		Элементы.алкДеревоНастроек.Развернуть(ЭлементСтроки.ПолучитьИдентификатор(), Истина);
	КонецЦикла;
	Для Каждого ЭлементСтроки Из алкДеревоДопПрав.ПолучитьЭлементы() Цикл
		Элементы.алкДеревоДопПрав.Развернуть(ЭлементСтроки.ПолучитьИдентификатор(), Истина);
	КонецЦикла;
