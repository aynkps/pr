...
Функция СтруктураСообщенияОбмена(ТелоЗапроса) Экспорт
	...
	РезультатПроверки = НастройкиТекущейБД.ИДБазыИсточник = ТелоЗапроса.to;	
	СтруктураСообщенияОбмена.Вставить("РезультатПроверки", РезультатПроверки);
	Возврат	 СтруктураСообщенияОбмена;
	
КонецФункции

Функция СтруктураОтветаУниверсальногоОбменаПоСсылкеУО(СсылкаУО, Данные) Экспорт
	...
	ТелоЗапроса = ЗаполненноеТелоЗапросаHTTP(ПараметрыПодключения.ИДБазыПриемник, НастройкиТекущейБД.ИДБазыИсточник, Данные);
...
Функция СтруктураТелаЗапросаHTTP()   Экспорт
	
	СтруктураОтветаHTTP = Новый Структура("to, front, data");
	//СтруктураОтветаHTTP.header = Новый Структура("to, front");
...
Функция ЗаполненноеТелоЗапросаHTTP(ИДБазыПриемник, ИДБазыИсточник, data, СерилизоватьXDTO = Истина)  Экспорт
	...
	ДанныеДляОтвета.to = ИДБазыПриемник;
	ДанныеДляОтвета.front = ИДБазыИсточник;
