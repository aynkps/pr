...
Функция ВнешняяОбработкаБСП(ИмяДопОбработки = "", СсылкаВнешнейОбработки = Неопределено)  Экспорт
	...
	Если ИмяДопОбработки <> "" Тогда
		ДопОбработкаСсылка = СсылкаВнешнейОбработкаБСП(ИмяДопОбработки);
	ИначеЕсли СсылкаВнешнейОбработки <> Неопределено Тогда
...
//РЗ - РезультатЗапроса Или ВыборкаИзРезультатаЗапроса 
Функция РезультатЗаписиДанныхЗапросаВТранзации(РЗ, СписокПолейДляЗаписи = "", СписокИсключаемыхПолей = "Ссылка", ОбменДаннымиЗагрузка = Истина, ПривилегированныйРежим = Истина, РежимОтладки = Ложь, ОбъемТранзакции = 500, РежимЗаписи = Неопределено, ОтключитьСтандартныеПроверки = Ложь)	Экспорт
...
//
Функция КаталогСуществует(ПутьККаталогу, СоздаватьЕслиНеСуществует = Истина) Экспорт
	...
	Попытка
		...
		Если КаталогНаДиске.Существует() Тогда
			Возврат Истина;
		ИначеЕсли  СоздаватьЕслиНеСуществует = Истина Тогда
...
Функция ЗапуститьФоновоеВыполнение(ИмяПроцедуры, МассивПараметров = Неопределено, ИДФоновогоЗадания = "", НаименованиеФоновогоЗадания = "") Экспорт
	
	Если ИДФоновогоЗадания = "" Тогда
		ИДФоновогоЗадания = Новый УникальныйИдентификатор;
	КонецЕсли;
	
	ФоновыеЗадания.Выполнить(ИмяПроцедуры, МассивПараметров, ИДФоновогоЗадания, НаименованиеФоновогоЗадания);
