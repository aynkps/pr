...
&НаКлиенте
Процедура ОткрытьФормуВнешнююОбработкиБСП(ИмяОбработки, ИмяВнешенйФормы = "Форма", ПараметрыОткрытия = Неопределено, ФормаВладелец = Неопределено, Уникальность = Неопределено, 
	...
	ОткрытьФормуВнешнейОбработки(, ИмяВнешенйФормы, ПараметрыОткрытия, ФормаВладелец, Уникальность, Окно, НавигацСсылка, СтруктураОповещенияОЗакрытии, РежимОткрытия, ИмяОбработки, Истина, ЭтоОбработка)	
КонецПроцедуры
...
Процедура ОткрытьФормуВнешнейОбработки(ПолныйПуть = "", ИмяВнешенйФормы = "Форма", ПараметрыОткрытия = Неопределено, ФормаВладелец = Неопределено, Уникальность = Неопределено, 
								Окно = Неопределено, НавигацСсылка = Неопределено, СтруктураОповещенияОЗакрытии = Неопределено, РежимОткрытия = Неопределено, ИмяОбработки = "", ЭтоБСП = Ложь, ЭтоОбработка = Истина)  Экспорт
	
	Если ЭтоБСП Тогда
		ИмяОбработки = пр_Общий.ПодключитьВнешнююОбработкуБСП(ИмяОбработки);
...
&НаКлиенте
Функция ПолучитьФормуПоПути(ПолныйПуть, ИмяОбработки = "", ИмяВнешенйФормы, ПараметрыОбработки = Неопределено, ВладелецФормы = Неопределено, Уникальность = Неопределено, Окно = Неопределено, НавигационнаяСсылка= Неопределено, ЭтоОбработка = Истина) Экспорт
