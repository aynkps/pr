...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора,
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		Если ПроверятьПорядокРасчетов
			...
			И МассивПорядковРасчета.Найти(ВыборкаТовары.ПорядокРасчетов) = Неопределено Тогда
			МассивПорядковРасчета.Добавить(ВыборкаТовары.ПорядокРасчетов);
		КонецЕсли;
	КонецЦикла;
	
	Если МассивЗаказов = Неопределено
		И ПроверятьПорядокРасчетов 
		И МассивПорядковРасчета.Количество() > 1 Тогда
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЕСТЬNULL(ДанныеДокументаТовары.ЗаказКлиента.ЭтоЗаказКакСчет, НЕОПРЕДЕЛЕНО) КАК ЭтоЗаказКакСчет
	|ПОМЕСТИТЬ ДанныеДокументаТовары
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.Товары КАК ДанныеДокументаТовары
	|ГДЕ
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаВидыЗапасов.ОбъектРасчетов.НаправлениеДеятельности КАК НаправлениеДеятельностиКонтрагента,
	|	ВЫБОР
	|		КОГДА &РеализацияПоЗаказу
	|				И &РеализацияПоНесколькимЗаказам
	...
	|	ТаблицаТовары.ОбъектРасчетов.НаправлениеДеятельности КАК НаправлениеДеятельностиКонтрагента,
	|	ВЫБОР
	|		КОГДА &РеализацияПоЗаказу
	|				И &РеализацияПоНесколькимЗаказам
	|			ТОГДА ТаблицаТовары.ЗаказКлиента
...
Функция ТекстЗапросаТаблицаЗаказыКлиентов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И НЕ ЕСТЬNULL(ТаблицаТовары.ЗаказКлиента.ЭтоЗаказКакСчет, НЕ &ИспользоватьРасширенныеВозможностиЗаказаКлиента)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ТекстЗапросаВтТаблицаВидыЗапасов(Запрос, ТекстыЗапроса)
	...
	|	ПО АналитикаРасчетов.Партнер = ВидыЗапасов.ОбъектРасчетов.Партнер
	|		И АналитикаРасчетов.Организация = ВидыЗапасов.ОбъектРасчетов.Организация
	|		И АналитикаРасчетов.Контрагент = ВидыЗапасов.ОбъектРасчетов.Контрагент
	|		И АналитикаРасчетов.Договор = ВидыЗапасов.ОбъектРасчетов.Договор
	|		И АналитикаРасчетов.НаправлениеДеятельности = ВидыЗапасов.ОбъектРасчетов.НаправлениеДеятельности
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК СпрНазначения
...
Функция ТекстЗапросаВтРаботыУслуги(Запрос, ТекстыЗапроса)
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК АналитикаРасчетов
	|	ПО АналитикаРасчетов.Партнер = ТаблицаТовары.ОбъектРасчетов.Партнер
	|		И АналитикаРасчетов.Организация = ТаблицаТовары.ОбъектРасчетов.Организация
	|		И АналитикаРасчетов.Контрагент = ТаблицаТовары.ОбъектРасчетов.Контрагент
	|		И АналитикаРасчетов.Договор = ТаблицаТовары.ОбъектРасчетов.Договор
	|		И АналитикаРасчетов.НаправлениеДеятельности = ТаблицаТовары.ОбъектРасчетов.НаправлениеДеятельности
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтТаблицаАналитикУчетаПартий КАК ТаблицаАналитикУчетаПартий
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстОплата = "
		...
		|ГДЕ
		|	Таблица.Ссылка В (&Ссылка)
		|	И Таблица.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет)
		...
		|	И Таблица.Ссылка.ОбъектРасчетовУпр = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
		|";
...
Процедура ОтразитьРезерв(Запрос, ТекстыЗапроса, Регистры)
	...
	|				И НЕ ЕСТЬNULL(ТоварыДокумента.ЗаказКлиента.ЭтоЗаказКакСчет, НЕ &ИспользоватьРасширенныеВозможностиЗаказа)
...
Функция ТекстЗапросаВтБонусныеБаллы(Запрос, ТекстыЗапроса)
	...
	|		ИЛИ ЕСТЬNULL(ТабличнаяЧасть.ЗаказКлиента.ЭтоЗаказКакСчет, НЕ &ИспользоватьРасширенныеВозможностиЗаказаКлиента))
	|";
