...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// ▼ ayan#bitrix1827_02 Оптимизировать загрузку Д_реалилзацияТоваровУслуг Кириллов П.С.  2022.11.27 ▼ Начало
	аян_ЭтоОбмен = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеСвойства,"ЭтоОбмен", Ложь);
	//#КонецВстави
	//#Удаление
	//Если Не ДополнительныеСвойства.ИзменилосьТолькоСостояниеПереходаПраваСобственности Тогда
	//#КонецУдаления
	//#Вставка
	Если аян_ЭтоОбмен Тогда
		//Пропускаем  СФ загружается после реализации и проводится
	ИначеЕсли Не ДополнительныеСвойства.ИзменилосьТолькоСостояниеПереходаПраваСобственности Тогда
	// ▲ ayan#bitrix1827_02 Оптимизировать загрузку Д_реалилзацияТоваровУслуг Кириллов П.С.  2022.11.27 ▲ Конец
	//#КонецВстави
		ПараметрыРегистрации = Документы.РеализацияТоваровУслуг.ПараметрыРегистрацииСчетовФактурВыданных(ЭтотОбъект);
